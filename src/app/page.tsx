    ...
    <main className="bg-black text-white">
      ...
      >
        ...
        <button
          ...
          className="absolute bottom-7 left-16 top-auto z-40 grid size-10 translate-y-0 place-items-center border border-white/20 bg-black/35 text-2xl backdrop-blur-md transition hover:border-[#018C55] hover:bg-[#018C55] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white sm:bottom-auto sm:left-8 sm:top-1/2 sm:size-14 sm:-translate-y-1/2 sm:text-3xl"
        ...
        <button
          type="button"
          onClick={() => showSlide(activeSlide + 1)}
          className="absolute bottom-7 left-28 right-auto top-auto z-40 grid size-10 translate-y-0 place-items-center border border-white/20 bg-black/35 text-2xl backdrop-blur-md transition hover:border-[#018C55] hover:bg-[#018C55] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white sm:bottom-auto sm:left-auto sm:right-8 sm:top-1/2 sm:size-14 sm:-translate-y-1/2 sm:text-3xl"
          aria-label="Ver imagen siguiente"
        >
          <span aria-hidden="true">›</span>
        </button>

        <div className="absolute bottom-7 right-5 z-40 flex items-center gap-2 sm:bottom-8 sm:right-10 sm:gap-3 lg:right-14">
          {slides.map((slide, index) => (
            <button
              key={slide.src}
              type="button"
              onClick={() => showSlide(index)}
              className="relative h-8 w-8 overflow-hidden focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white sm:w-12"
